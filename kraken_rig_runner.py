...
class ema_runner(kr.runner.runner):
	...
	def process_candle(self, candles):
		...
		if not self.context['invested']:
			if ema3 > ema8 and len(cur_buy_orders) == 0:
				...
				#self.record(cc['start'], {'buy_order_price': cc['low'], 'buy_order_volume': 0.15})
...
class lag_runner(kr.runner.runner):
	...
	def process_candle(self, candles):
		...
		if not self.context['invested']:
			...
			if macd < 0 and lag['lag'][-1] < 0.75 and ema14_prev < ema34_prev and ema14 > ema34 and not self.context['down_turn']:
	...
	def on_buy_order_partially_filled(self, order, new_order):
		...
		volume_in_sought_currency = self.wallet.balance[order['currency_sought']]
		self.remove_buy_order(new_order)
		new_sell_order = self.place_sell_order(self.currency_offered, self.currency_sought, price, volume_in_sought_currency)
		self.place_stop_limit(stop, stop - 0.00001, new_sell_order)
		self.context['invested'] = True
	...
	def laguerre_series(self, close, gamma=0.6):
		...
		lag_frame = {'L0': L0,'L1': L1,'L2': L2,'L3': L3,'CU': CU,'CD': CD, 'rsi': rsi,'lag': lag, 'close': [close[len(close)-1]]}
		return lag_frame
...
start_date = '2017-06-01 00:00:00.00'
end_date = '2017-07-01 00:00:00.00'
...
runner = ema_runner('usd', 'btc', wallet)
#runner = lag_runner(wallet, 'usd', 'btc')
#market, start_date, end_date, runner=None, frequency=60.0)
processor = kr.processor.processor('XBTUSD', start_date, end_date, runner, frequency=60)
processor.run()
